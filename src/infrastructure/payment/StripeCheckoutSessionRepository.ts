...
export class StripeCheckoutSessionRepository implements CheckoutSessionRepository{
  ...
  async createOneTimeCheckoutSession(amount: number, paymentUser?: PaymentUser): Promise<string> {
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: process.env.STRIPE_SUCCESSS_URL,
      cancel_url: process.env.STRIPE_CANCEL_URL,
    });
    return session.id
  }

  async createSubscriptionCheckoutSession(plan_id: string, paymentUser: PaymentUser): Promise<string> {
    const session = await stripe.checkout.sessions.create({
      payment_method_types: ['card'],
  ...
  async find(checkout_session_id: string): Promise<CheckoutSession | undefined> {
    const session = await stripe.checkout.sessions.retrieve(checkout_session_id)
    const paymentUser = await this.paymentUserRepository.findPaymentUser(session.customer)
